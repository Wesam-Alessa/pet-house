...
class AddNewToolScreen extends StatefulWidget {
  const AddNewToolScreen({Key? key}) : super(key: key);

  @override
  State<AddNewToolScreen> createState() => _AddNewToolScreenState();
}

class _AddNewToolScreenState extends State<AddNewToolScreen> {
  ...
  void getCategories(List<PetToolCategoryModel> list) {
    if (categoryItems.isEmpty) {
      for (PetToolCategoryModel model in list) {
  ...
  Future<ToolSeller> getSeller() async {
    var shared = await SharedPreferences.getInstance();
    var value = shared.getString('user') ?? '';
    Map<String, dynamic> result = jsonDecode(value);
    var seller = ToolSeller.fromJson(result);
    return seller;
  }
  ...
  Widget build(BuildContext context) {
    return BlocProvider<ToolsCubit>(
      create: (context) => getIt.get<ToolsCubit>()..getToolsCategories(context),
      child: BlocBuilder<ToolsCubit, ToolsState>(builder: (context, state) {
        if (state is LoadingToolsCategoriesState || state is LoadingToolsState) {
          return Scaffold(
        ...
        if (state is ErrorLoadingToolsCategoriesState) {
          return Scaffold(
              backgroundColor: AppColors().backgroundColorScaffold,
              appBar: AppBar(
                leading: IconButton(
        ...
        if (state is LoadedToolsCategoriesState ) {
          ...
          return Scaffold(
            ...
            appBar: AppBar(
              ...
              elevation: 0.0,
              centerTitle: true,
              title: Text(AppLocalizations.of(context)!.add_new_tool,
                  style: TextStyles.titleTextStyle),
              actions: [
                IconButton(
                  onPressed: () async {
                    ...
                    await getSeller().then((value) {
                      BlocProvider.of<ToolsCubit>(context, listen: false)
                          .addNewTool(
                              context,
                              PetToolModel(
                                image: const [
